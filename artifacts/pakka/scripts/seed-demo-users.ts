...
async function listAllAuthUsers(): Promise<Map<string, string>> {
  ...
  while (true) {
    ...
    for (const u of data.users) {
      if (u.phone) {
        const normalized = u.phone.startsWith("+") ? u.phone : "+" + u.phone;
        phoneToId.set(normalized, u.id);
      }
    }
...
async function seed() {
  ...
  for (const u of DEMO_USERS) {
    ...
    if (existing.has(u.phone)) {
      ...
      if (authErr) {
        const isPhoneExists =
          authErr.status === 422 ||
          (authErr as { code?: string }).code === "phone_exists" ||
          authErr.message?.toLowerCase().includes("phone_exists");
        if (!isPhoneExists) bail(`createUser ${u.phone}`, authErr);
        // Phone already registered but wasn't in our initial snapshot
        // (e.g. stored without leading "+"). Re-fetch to get the real UUID.
        console.warn(`⚠️   phone_exists for ${u.phone} — re-fetching from auth…`);
        const refetched = await listAllAuthUsers();
        const refetchedId = refetched.get(u.phone);
        if (!refetchedId) {
          bail(
            `createUser ${u.phone} — phone_exists but UUID not found after re-fetch`,
            authErr,
          );
        }
        userId = refetchedId;
    ...
    if (
      u.role === "client" &&
      (u.available !== undefined || u.locked !== undefined)
    ) {
  ...
  const job2 = await upsertJob({
    ...
    description:
      "Complete rewiring of two bathrooms including safety check and fixture install.",
    category: "electrical",
  ...
  const job3 = await upsertJob({
    ...
    description:
      "Full office repaint — walls and ceiling. Premium washable paint specified.",
    category: "painting",
  ...
  await upsertMilestone(job4, 1, {
    title: "Frame & assembly",
    amount: 15000,
    status: "pending",
  });
  await upsertMilestone(job4, 2, {
  ...
  const job5 = await upsertJob({
    ...
    description:
      "Blocked drainage in kitchen + bathroom. Needs rooter + pipe replacement.",
    category: "plumbing",
    location_text: "Bandra West, Mumbai",
    total_budget: 15000,
    status: "open",
  ...
  for (const app of [
    ...
    if (!existingApp) {
      ...
      if (appErr)
        bail(`job_applications insert job5 worker=${app.worker_id}`, appErr);
    }
  }
  ...
  const job6 = await upsertJob({
    ...
    description:
      "Complete gut-and-remodel of master bathroom. Tiling, plumbing, fixtures.",
    category: "plumbing",
  ...
  console.log(
    `   In-escrow lock: ${fmt(locked)}  ${ledgerOk ? "✅ balanced" : "❌ MISMATCH"}`,
  );
  if (!ledgerOk) {
  ...
  console.log(
    `   ℹ️  To log in without real SMS, configure Test OTP numbers in`,
  );
  console.log(
    `      Supabase Dashboard → Auth → Sign In/Up → Phone → Test OTPs`,
  );
