...
async function fetchMilestonesData(jobId: string) {
  ...
  const workerName = (workerRes.data?.profiles as unknown as { full_name: string } | null)?.full_name ?? "Worker";
      ...
      >
        <DialogContent>
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => { setConfirmDialog(null); setDisputeReason(""); }}
              disabled={isPending}
            >
              Cancel
            </Button>
  ...
  return (
    <div
      className={cn(
        ...
        (m.status === "approved" || m.status === "released") && "border-emerald-200 bg-emerald-50/30",
      )}
    >
