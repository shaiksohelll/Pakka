-- CodeRabbit PR #1 Tier B Perf findings.
-- Part 1: Wrap auth.uid() in (SELECT auth.uid()) to force initplan
--         evaluation (once per query) instead of per-row. Identical predicate
--         semantics. Closes Supabase advisor lint 0019 (auth_rls_initplan).
-- Part 2: Add missing FK indexes on escrow_ledger.{from_wallet, to_wallet, job_id}.

-- ========== Part 1: auth.uid() initplan wrap ==========
...
CREATE POLICY "disputes_insert_participants" ON public.disputes
  ...
  WITH CHECK ((raised_by = (SELECT auth.uid())) AND is_job_participant(job_id));
...
CREATE POLICY "jobs_insert_client" ON public.jobs
  FOR INSERT TO authenticated
  WITH CHECK ((client_id = (SELECT auth.uid())) AND (worker_id IS NULL));
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE j.id = materials.job_id
        AND ((j.client_id = (SELECT auth.uid())) OR (j.worker_id = (SELECT auth.uid())))
    )
  );
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE m.id = proofs.milestone_id
        AND j.worker_id = (SELECT auth.uid())
    )
  );
...
CREATE INDEX IF NOT EXISTS escrow_ledger_from_wallet_idx
  ON public.escrow_ledger (from_wallet);

CREATE INDEX IF NOT EXISTS escrow_ledger_to_wallet_idx
  ON public.escrow_ledger (to_wallet);
